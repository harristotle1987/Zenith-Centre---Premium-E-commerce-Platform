...
const newProductsStr = `\nexport const REAL_PRODUCTS: Record<string, any[]> = ${JSON.stringify(products, null, 2)};\n`;

let serverTs = fs.readFileSync('server.ts', 'utf8');

// Remove existing REAL_PRODUCTS definitions
serverTs = serverTs.replace(/const REAL_PRODUCTS: Record<string, any\[\]> = \{[\s\S]*?\]\n        \};/g, '');
serverTs = serverTs.replace(/const REAL_PRODUCTS: Record<string, any\[\]> = \{[\s\S]*?\]\n      \};/g, '');

// Insert at top
serverTs = serverTs.replace('const DB_URL =', newProductsStr + '\nconst DB_URL =');
