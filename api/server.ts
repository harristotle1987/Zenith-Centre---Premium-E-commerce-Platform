...
io.on('connection', (socket) => {
    ...
    try {
      ...
      for (const item of items) {
        const result = await sql`
          ...
          RETURNING stock_quantity as stock, id
        `;

        // Emit socket event for real-time stock update
        if (io && result.length > 0) {
          const productRes = await sql`SELECT p.*, d.name as department FROM products p JOIN departments d ON p.department_id = d.id WHERE p.id = ${item.product_id}`;
          if (productRes.length > 0) {
            io.emit('productUpdated', productRes[0]);
          }
        }
      }
  ...
  app.get('/api/settings/public', async (req, res) => {
    try {
      const settings = await sql`SELECT * FROM settings WHERE key IN ('heroImageUrl', 'featuredImageUrl1', 'featuredImageUrl2', 'featuredImageUrl3', 'featuredImageUrl4', 'minStockThreshold')`;
  ...
  app.put('/api/products/:id/stock', authenticateToken, authorizeRole(['super_admin', 'staff', 'secretary', 'manager', 'counter_staff']), async (req, res) => {
    try {
      ...
      if (io) {
        const productWithDept = await sql`SELECT p.*, d.name as department FROM products p JOIN departments d ON p.department_id = d.id WHERE p.id = ${id}`;
        if (productWithDept.length > 0) {
          io.emit('productUpdated', productWithDept[0]);
        }
      }
